...
# Install gcc (necessary for .compile() with PyTorch 2.0) and openssh-client & nano for devcontainers
USER root
...
RUN --mount=type=cache,target=${MAMBA_ROOT_PREFIX}/pkgs,id=conda-linux/amd64 micromamba create --name research --yes --file /locks/conda-lock.yml
# Run this in a seperate step to still keep the docker cache for future builds
