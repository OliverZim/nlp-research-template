...
# This Dockerfile is for machines with the ppc64le processor architecure

# Load micromamba container to copy from later
FROM --platform=linux/ppc64le mambaorg/micromamba:1.3.1 as micromamba
...
ARG TARGETPLATFORM
COPY --chown=$MAMBA_USER:$MAMBA_USER ppc64le.conda-lock.yml /locks/conda-lock.yml
...
RUN --mount=type=cache,target=${MAMBA_ROOT_PREFIX}/pkgs,id=conda-linux/ppc64le micromamba create --name research --yes --file /locks/conda-lock.yml
